...
@section('title', 'Seller Dashboard - SEAPEDIA')
...
<div class="card p-8 bg-white border border-sand-300 shadow-warm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="md:col-span-2 space-y-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-sand-700 leading-relaxed">{{ auth()->user()->store->description ?? 'Tidak ada deskripsi toko.' }}</p>
                </div>
            </div>
            ...
            <div class="p-5 rounded-sea border border-sand-300 bg-sand-50/50 flex flex-col justify-between">
                ...
                <div class="mt-4 flex gap-2">
                    <a href="{{ route('seller.store.edit') }}" class="btn btn-secondary btn-sm w-full">
                        Edit Profil Toko
                    </a>
                </div>
                <p class="text-[10px] text-sand-500 mt-4">Terdaftar sejak {{ auth()->user()->store->created_at->format('d M Y') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6 border-t border-sand-200">
            <div class="card p-6 bg-sand-50/30 border border-sand-300 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-black text-navy-800">{{ auth()->user()->store->products->count() }} Produk</p>
                </div>
    ...
    @else
        ...
        <div class="bg-coral-50 border border-coral-200 p-8 rounded-sea mb-8 flex flex-col items-center text-center">
            ...
            <a href="{{ route('seller.store.create') }}" class="btn btn-primary btn-sm">
