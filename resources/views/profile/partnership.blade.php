...
@section('title', 'Pendaftaran Kemitraan - SEAPEDIA')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12" x-data="{ selectedRole: '' }">
    ...
    <form action="{{ route('profile.partnership.store') }}" method="POST" class="space-y-8">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            
            <!-- Seller Partnership Card -->
            <label class="relative block cursor-pointer select-none group @if(in_array('seller', $roles)) pointer-events-none @endif">
                <input type="radio" name="role" value="seller" class="sr-only"
                       x-model="selectedRole"
                       @if(in_array('seller', $roles)) disabled @endif>
                ...
                <div :class="selectedRole === 'seller' ? 'border-coral-400 bg-coral-50/10 shadow-warm-lg ring-2 ring-coral-400/20' : 'border-sand-300 bg-white hover:border-coral-300'"
                     class="h-full p-6 rounded-[20px] border transition-all duration-300 flex flex-col justify-between min-h-[220px] @if(in_array('seller', $roles)) opacity-55 cursor-not-allowed bg-sand-50 @endif">
            ...
            <label class="relative block cursor-pointer select-none group @if(in_array('driver', $roles)) pointer-events-none @endif">
                <input type="radio" name="role" value="driver" class="sr-only"
                       x-model="selectedRole"
                       @if(in_array('driver', $roles)) disabled @endif>
                ...
                <div :class="selectedRole === 'driver' ? 'border-navy-400 bg-navy-50/10 shadow-warm-lg ring-2 ring-navy-400/20' : 'border-sand-300 bg-white hover:border-navy-300'"
                     class="h-full p-6 rounded-[20px] border transition-all duration-300 flex flex-col justify-between min-h-[220px] @if(in_array('driver', $roles)) opacity-55 cursor-not-allowed bg-sand-50 @endif">
