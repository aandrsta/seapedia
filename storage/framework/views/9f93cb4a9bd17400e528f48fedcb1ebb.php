    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 pb-8 border-b border-navy-800">
            ...
            <div class="flex flex-wrap gap-x-8 gap-y-3">
                <a href="<?php echo e(route('home')); ?>" class="text-xs font-black uppercase tracking-widest text-sand-300 hover:text-white transition-colors">HOME</a>
                <a href="<?php echo e(route('products.index')); ?>" class="text-xs font-black uppercase tracking-widest text-sand-300 hover:text-white transition-colors">KATALOG</a>
                <?php if(auth()->guard()->check()): ?>
                    <?php if(!auth()->user()->roles->contains('name', 'seller')): ?>
                        <a href="<?php echo e(route('select-role')); ?>" class="text-xs font-black uppercase tracking-widest text-sand-300 hover:text-white transition-colors">BUKA TOKO</a>
                    <?php endif; ?>
                <?php else: ?>
                    <a href="<?php echo e(route('register')); ?>" class="text-xs font-black uppercase tracking-widest text-sand-300 hover:text-white transition-colors">BUKA TOKO</a>
                <?php endif; ?>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 text-[10px] text-navy-400 font-bold uppercase tracking-wider">
            <span>&copy; <?php echo e(date('Y')); ?> SEAPEDIA. ALL RIGHTS RESERVED.</span>
        </div>
    </div>
</footer>
<?php /**PATH /home/dini/Sites/seapedia/resources/views/components/footer.blade.php ENDPATH**/ ?>